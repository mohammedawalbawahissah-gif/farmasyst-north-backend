...
class CreditworthinessView(APIView):
    ...
    def post(self, request):
        ...
        # Only admins or the farmer themselves can run this
        user = request.user
        ...
        try:
            ...
            for farm in farms:
                ...
                recent_logs = list(
                    ...
                    .values('date', 'mortality', 'feed_kg', 'flock_count' if False else 'broiler_count',
                            'layer_count', 'eggs_collected', 'medication_given', 'notes')
                )
                # Compute mortality rate from logs
                total_flock = sum(
            ...
            # Credit history
            from credit.models import CreditApplication, CreditAgreement
            past_apps = list(
...
class DiseaseDetectionView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not logs:
                return Response({'detail': 'No activity logs found for this farm. Start logging daily activity to enable disease detection.'}, status=400)
            ...
            system_prompt = (
                ...
                "Analyse farm activity logs to identify early health risks and disease indicators. "
            ...
            raw = call_claude(system_prompt, messages, max_tokens=1000)
            clean = raw.strip()
            if clean.startswith('```'):
                clean = clean.split('\n', 1)[1].rsplit('```', 1)[0].strip()
            result = json.loads(clean)
...
class AIChatView(APIView):
    ...
    def post(self, request):
        ...
        # Load history (last 20 messages to stay within context)
        history = list(
